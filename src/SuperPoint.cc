...
SPDetector::SPDetector(const std::string & modelPath, float threshold, bool nms, int minDistance, bool cuda) :
		...
		detected_(false)
{
	printf("modelPath=%s thr=%f nms=%d cuda=%d", modelPath.c_str(), threshold, nms?1:0, cuda?1:0);
	if(modelPath.empty())
	{
		printf("Model's path is empty!");
		return;
	}
	...
	{
		printf("Cuda option is enabled but torch doesn't have cuda support on this platform, using CPU instead.");
	}
